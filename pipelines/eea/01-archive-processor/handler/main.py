...
def handler(context, event):
    ...
    try:
        ...
        natura_id_field = b['natura_id_field']
        normalized_natura2000_properties = b['normalized_natura2000_properties']
        version = b['version']
        ...
        for f in shpfs:
            ...
            for sr in reader.iterShapeRecords():
                ...
                geom_id = "{0}:{1}".format(source_id,geom_properties[natura_id_field])
                simplify_tolerance = 1

                #siteid = geom_properties[natura_id_field]
                #if not (siteid.startswith('FR') or siteid.startswith('ES')):
                #    continue

                if 'RELEASE_DA' in geom_properties and isinstance(geom_properties['RELEASE_DA'],date):
                    geom_properties['RELEASE_DA'] = geom_properties['RELEASE_DA'].isoformat()
                if 'SITENAME' in geom_properties and isinstance(geom_properties['SITENAME'],bytes):
                    geom_properties['SITENAME'] = geom_properties['SITENAME'].decode('ISO-8859-1')

                properties = {
                    "natura": geom_properties,
                    "crs": {
                        "type": "EPSG",
                        "properties": {
                            "code": crs_code
                ...
                for prop in normalized_natura2000_properties:
                    value = geom_properties.get(normalized_natura2000_properties[prop]['sourceProp'])
                    # Get value in SI base unit if needed
                    if 'coefSI' in normalized_natura2000_properties[prop]:
                        value = eval("{0}{1}".format(value,normalized_natura2000_properties[prop]['coefSI']))
                    if 'valueMap' in normalized_natura2000_properties[prop]:
                        if value in normalized_natura2000_properties[prop]['valueMap']:
                            value =  normalized_natura2000_properties[prop]['valueMap'][value]
                    if 'evalMethod' in normalized_natura2000_properties[prop]:
                        value = getattr(value,normalized_natura2000_properties[prop]['evalMethod'])()
                    properties[prop] = value
    
                # forge a GeoJSON feature
                natura = dict(_id=geom_id, type="Feature", properties=properties)

                while simplify_tolerance < 100:
                    try:
                        natura['geometry'] = shape(sr.shape.__geo_interface__) \
                                                .simplify(simplify_tolerance,preserve_topology=True) \
                                                .__geo_interface__
                        value = json.dumps(natura).encode("utf-8-sig")
                        p.produce(target_topic, value)
                        break
                    except KafkaException as e:
                        # If the message is too large let's try to simplify the geometry a little more
...
class FunctionConfig(object):
    ...
    accepted_source_id = 'natura2000:eea'

    target_files = [
        "Natura2000_end2017_epsg3035.shp"]

    source_crs_epsg_code = 3035
...
class Helpers(object):
    ...
    def parse_body(context, body):
        ...
        if not 'naturaIdField' in body:
            context.logger.warn_with('Missing \'naturaIdField\' attribute !')
            raise NuclioResponseError(
                'Missing \'naturaIdField\' attribute !',requests.codes.bad)
        natura_id_field = body['naturaIdField']
        ...
        normalized_natura2000_properties = body['normalizedProperties']
        ...
        b = {
            ...
            "source_id": source_id,
            "natura_id_field": natura_id_field,
            "normalized_natura2000_properties": normalized_natura2000_properties,
            "version": version
        }
